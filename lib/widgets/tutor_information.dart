...
class TutorInformation extends StatefulWidget {
  ...
  @override
  State<TutorInformation> createState() => _TutorInformationState();
}
...
class _TutorInformationState extends State<TutorInformation> {
  ...
  _fetchTutorData() {
    FirebaseFirestore.instance.collection('tutors').doc(sharedPreferences!.getString("uid")).get().then((snap)
    {
      ...
      locality = snap.data()!["locality"].toString();
      setState(() {
  ...
  Widget build(BuildContext context) {
    distance = getDistanceFromSharedPreference(sharedPreferences!.getString("uid").toString())/1000;
    duration = getDurationFromSharedPreference(sharedPreferences!.getString("uid").toString())/60;
        ...
        Column(
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                _buildTextIcon(Icons.location_on_outlined, Colors.cyan, locality),
